"""
File: combine_folder_multiprocess.py

Author: Watchful1
Date Modified: June 2nd, 2024

Description:
    This script iterates through Zstandard-compressed NDJSON files, such as the Pushshift Reddit dumps, loading each line
    and filtering based on the criteria specified via command-line arguments. Matching records are written out into
    separate monthly files. After processing all NDJSON files, the script merges the intermediate outputs into a final file.

    The script assumes that files are named in chronological order and prefixed with 'RS_' or 'RC_', following the standard
    Pushshift dump naming conventions.

Features:
    - Utilizes multiple processes in parallel to maximize disk read speeds and decompression performance.
    - Saves progress after processing each file, allowing it to resume where it left off.
...
Attribution:
    Original script created by Watchful1. This documentation was reformatted for clarity but does not modify the script's functionality.
    The link to the script can be found here: https://github.com/Watchful1/PushshiftDumps/blob/master/scripts/combine_folder_multiprocess.py
"""
